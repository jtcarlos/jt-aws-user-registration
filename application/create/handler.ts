...
export const handler = async (
  context: ICreatePayload
): Promise<IDefaultReturn> => {
  try {
    const { headers, body, httpMethod } = context;
    const contentType = headers['Content-Type'];

    if (httpMethod !== methods.POST)
      throw new HTTPError(405, `Method Not Allowed: ${httpMethod}`);

    if (contentType !== 'application/json')
      throw new HTTPError(406, `Content Not Acceptable: ${contentType}`);
    ...
    const decodedProfilePicture = Buffer.from(
